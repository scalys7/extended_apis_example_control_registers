...
#include <vmcs_cr/vmcs_cr.h>
#include <exit_handler_cr/exit_handler_cr.h>

std::unique_ptr<vcpu>
vcpu_factory::make_vcpu(vcpuid::type vcpuid, user_data *data)
{
    auto &&my_vmcs = std::make_unique<vmcs_cr>();
    auto &&my_exit_handler = std::make_unique<exit_handler_cr>();
